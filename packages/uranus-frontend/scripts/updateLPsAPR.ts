...
const BLOCK_SUBGRAPH_ENDPOINT = process.env.REACT_APP_BLOCK_URL // FIXME env var graphql block
const STREAMING_FAST_ENDPOINT = process.env.REACT_APP_EXCHANGE_URL // FIXME env var graphql exchange
...
const fetchAndUpdateLPsAPR = async () => {
  ...
  const lowerCaseAddresses = farmsConfig
    .filter((farm) => farm.pid > 250) // FIXME script
