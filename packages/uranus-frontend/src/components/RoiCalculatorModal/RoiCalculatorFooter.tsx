...
import { Flex, Box, Text, ExpandableLabel, LinkExternal, Grid, HelpIcon, useTooltip } from 'uranus-uikit'
...
const RoiCalculatorFooter: React.FC<RoiCalculatorFooterProps> = ({
  ...
  const {
    ...
    <>
      <Text>
        {t(
          'The Multiplier represents the proportion of TAN rewards each farm receives, as a proportion of the TAN produced each block.',
        )}
      </Text>
      <Text my="24px">
        {t('For example, if a 1x farm received 1 TAN per block, a 40x farm would receive 40 TAN per block.')}
      </Text>
      ...
      {isExpanded && (
        <Box px="8px">
          <Grid gridTemplateColumns="2.5fr 1fr" gridRowGap="8px" gridTemplateRows={`repeat(${gridRowCount}, auto)`}>
            ...
            <Text color="textSubtle" small>
              {isFarm ? t('Base APR (TAN yield only)') : t('APR')}
            </Text>
            <Text small textAlign="right">
              {apr.toFixed(2)}%
            </Text>
              ...
              <>
                ...
                <Flex justifyContent="flex-end" alignItems="flex-end">
                  ...
                  <span ref={multiplierRef}>
                    <HelpIcon color="primary" width="16px" height="16px" />
                  </span>
