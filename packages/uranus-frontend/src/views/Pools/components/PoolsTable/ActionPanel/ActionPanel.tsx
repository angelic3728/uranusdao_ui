...
import { getAddress, getTanVaultAddress } from 'utils/addressHelpers'
import { BIG_ZERO } from 'utils/bigNumber'
import { registerToken } from 'utils/wallet'
import { getBalanceNumber, getFullDisplayBalance } from 'utils/formatBalance'
import { convertSharesToTan, getPoolBlockInfo } from 'views/Pools/helpers'
...
const ActionPanel: React.FC<ActionPanelProps> = ({ account, pool, userDataLoaded, expanded, breakpoints }) => {
  ...
  const tanVaultContractAddress = getTanVaultAddress()
  ...
  const poolStakingTokenBalance = isAutoVault
    ? tanAsBigNumber.plus(stakingTokenBalance)
  ...
  const getTotalStakedBalance = () => {
    if (isAutoVault) {
      return getBalanceNumber(totalTanInVault, stakingToken.decimals)
    }
    if (isManualtanPool) {
      const manualTanTotalMinusAutoVault = new BigNumber(totalStaked).minus(totalTanInVault)
      return getBalanceNumber(manualTanTotalMinusAutoVault, stakingToken.decimals)
    }
    ...
    <StyledActionPanel expanded={expanded}>
      <InfoSection>
        ...
        {poolContractAddress && (
          <Flex mb="8px" justifyContent={['flex-end', 'flex-end', 'flex-start']}>
            <LinkExternal
              href={`${BASE_EXPLORER_URLS[chainId]}/address/${isAutoVault ? tanVaultContractAddress : poolContractAddress}`}
              bold={false}
            >
              {t('View Contract')}
            </LinkExternal>
      ...
      <ActionContainer>
        {showSubtitle && (
          <Text mt="4px" mb="16px" color="textSubtle">
            {isAutoVault ? t('Automatic restaking') : `${t('Earn')} TAN ${t('Stake').toLocaleLowerCase()} TAN`}
          </Text>
        )}
        {pool.isAutoVault ? (
