...
import { fetchTanVaultUserData } from 'state/pools'
...
import { convertTanToShares, convertSharesToTan } from '../../helpers'
...
const VaultStakeModal: React.FC<VaultStakeModalProps> = ({
  ...
  const usdValueStaked = new BigNumber(stakeAmount).times(tanPriceUsdc)
  const formattedUsdValueStaked = tanPriceUsdc.gt(0) && stakeAmount ? formatNumber(usdValueStaked.toNumber()) : ''
  ...
  const handleWithdrawal = async (convertedStakeAmount: BigNumber) => {
    setPendingTx(true)
    const shareStakeToWithdraw = convertTanToShares(convertedStakeAmount, pricePerFullShare)
    // trigger withdrawAll function if the withdrawal will leave 0.000001 TAN or less
    ...
    if (isWithdrawingAll) {
      try {
        const tx = await callWithGasPrice(tanVaultContract, 'withdrawAll', undefined, callOptions)
  ...
  const handleDeposit = async (convertedStakeAmount: BigNumber) => {
    ...
    try {
      // .toString() being called to fix a BigNumber error in prod
      // as suggested here https://github.com/ChainSafe/web3.js/issues/2077
      const tx = await callWithGasPrice(tanVaultContract, 'deposit', [convertedStakeAmount.toString()], callOptions)
      const receipt = await tx.wait()
      if (receipt.status) {
        toastSuccess(
          t('Staked!'),
  ...
  if (showRoiCalculator) {
    return (
      <RoiCalculatorModal
        ...
        stakingTokenBalance={tanAsBigNumber.plus(stakingMax)}
  ...
  return (
    ...
    >
      ...
      <BalanceInput
        ...
        currencyValue={isBlindMode() ? null : (tanPriceUsdc.gt(0) && `~${formattedUsdValueStaked || 0} USD`)}
