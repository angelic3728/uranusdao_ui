...
import { Box, Flex, FlexProps, Skeleton, Text } from 'uranus-uikit'
import { useTranslation } from 'contexts/Localization'
import { BetPosition, NodeRound, Round } from 'state/types'
import { formatUsdv2, formatCrov2, getRoundPosition, getPriceDifference } from '../../helpers'
import { formatCro, formatUsd } from '../History/helpers'
...
const getPrizePoolAmount = (totalAmount: PrizePoolRowProps['totalAmount']) => {
  ...
  return formatCrov2(totalAmount)
}
...
export const PrizePoolRow: React.FC<PrizePoolRowProps> = ({ totalAmount, ...props }) => {
  ...
  return (
    <Row {...props}>
      ...
      <Text bold>{`${getPrizePoolAmount(totalAmount)} CRO`}</Text>
    </Row>
  )
}
  ...
  return (
    <Row height="18px" {...props}>
      ...
      <Flex alignItems="center">
        ...
        <Text mx="4px">|</Text>
        <Text fontSize="12px" lineHeight="18px">{`${formatCro(amount)} CRO`}</Text>
      </Flex>
    </Row>
  )
}
...
export const PrizePoolHistoryRow: React.FC<PrizePoolHistoryRowProps> = ({ totalAmount, ...props }) => {
  ...
  return (
    <Row {...props}>
      <Text bold>{t('Prize Pool')}:</Text>
      <Text bold>{`${getPrizePoolAmountHistory(totalAmount)} CRO`}</Text>
    </Row>
  )
}
