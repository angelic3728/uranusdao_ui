...
import { Price } from 'uranus-swap-sdk'
import { Flex, Text } from 'uranus-uikit'
import { multiplyPriceByAmount } from 'utils/prices'
import { useTranslation } from 'contexts/Localization'
import { formatCro } from '../helpers'
...
const SummaryRow: React.FC<SummaryRowProps> = ({ type, summary, croUsdcPrice }) => {
  ...
  const amountInUsd = multiplyPriceByAmount(croUsdcPrice, displayAmount)
    ...
    <>
      ...
      <Flex>
        ...
        <Flex flex="3" flexDirection="column">
          <Text bold fontSize="20px" color={color}>
            {`${summaryTypeSigns[type]}${formatCro(displayAmount)} CRO`}
          </Text>
          <Text fontSize="12px" color="textSubtle">
