...
const LotteryCardContent = () => {
  ...
  const tanPriceUsdcAsString = usePriceTanUsdc().toString()

  const tanPrizesText = t('%cakePrizeInUsd% in TAN prizes this round', { cakePrizeInUsd: tanPriceUsdcAsString })
  const [pretext, prizesThisRound] = tanPrizesText.split(tanPriceUsdcAsString)
  ...
  useEffect(() => {
    ...
    const fetchCurrentLotteryPrize = async () => {
      ...
      const prizeInUsdc = tanPriceUsdc.times(amountCollectedInCake)
  ...
  }, [lotteryId, slowRefresh, setCurrentLotteryPrize, tanPriceUsdc])
