...
import { Flex, Heading, Text, Button, Link, useMatchBreakpoints } from 'uranus-uikit'
import { useTranslation } from 'contexts/Localization'
import { isCountdown } from 'utils'

import { StyledBigHeading } from '../StyledHeadings'
import { TanTokenIntroData } from './data'
...
const TanTokenIntro = () => {
  ...
  const { headingText, items } = TanTokenIntroData
              ...
              >
                <Flex flexDirection={['column', 'column', 'column', 'row']} mb="10px">
                  ...
                  <Text style={{ alignSelf: 'center', fontWeight: 'bold' }} height="30px" lineHeight="34px" width="100%">
                    <img
                      src="/images/home/tan-info/aquaVector.svg"
                      alt="star1"
                      height="18px"
                      width="18px"
                      style={{
          ...
          <Flex justifyContent={isDesktop ? 'unset' : 'center'}>
            {
              isCountdown() ? (
                <>
                  ...
                  <>
                    <Link external href="https://docs.tan.finance/litepaper">
                      <StyledButton mr={10}>{t('White Paper')}</StyledButton>
                    </Link>
                    <Link external href="https://docs.tan.finance/litepaper/token-economics">
                      <StyledButton variant="secondary">{t('TAN Tokenomics')}</StyledButton>
                    </Link>
                  </>
              )
            }
