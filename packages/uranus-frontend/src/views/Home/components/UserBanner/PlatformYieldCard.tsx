...
import { Card, CardBody, Flex, Skeleton, Text } from 'uranus-uikit'
...
const PlatformYieldCard = () => {
  ...
  useEffect(() => {
    const fetchHarvestedData = async () => {
      ...
      if (harvested?.totalClaimAmount || harvested?.totalTanVaultProfit) {
        setHarvestedSum(Number(harvested.totalClaimAmount) + Number(harvested.totalTanVaultProfit))
      }
    }
    if (account) {
      fetchHarvestedData()
  ...
  const totalPlatformYield = (getBalanceNumber(new BigNumber(harvestedSum), tokens.tan.decimals) + farmEarningsSum) * tanPriceUsdc.toNumber()
