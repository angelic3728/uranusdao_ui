...
import { AutoRenewIcon, Button, Card, CardBody, Flex, Skeleton, Text, Link, ArrowForwardIcon } from 'uranus-uikit'
...
const HarvestCard = () => {
  ...
  // const tanPriceUsdc = new BigNumber(0) // XXX debug hard code for landing banner
  const earningsUsdc = new BigNumber(farmEarningsSum).multipliedBy(tanPriceUsdc)
  ...
  const hastanPoolToCollect = numTotalToCollect - numFarmsToCollect > 0

  const earningsText = getEarningsText(numFarmsToCollect, hastanPoolToCollect, earningsUsdc, t)
  ...
  const harvestAllFarms = useCallback(async () => {
    ...
    for (const farmWithBalance of farmsWithStakedBalance) {
      try {
        ...
        toastSuccess(
          `${t('Harvested')}!`,
          t('Your %symbol% earnings have been sent to your wallet!', { symbol: 'TAN' }),
        )
      } catch (error) {
