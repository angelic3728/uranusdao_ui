...
const SwapBody = styled(AppBody)<{ show: boolean; contentHeight: number }>`
...
export default function Swap({ history }: RouteComponentProps) {
  ...
  useEffect(() => {
    if (contentRef && contentRef.current)
      new ResizeObserver(() => {
        if (contentRef && contentRef.current) setContentHeight(contentRef.current.clientHeight)
        ...
        <SwapContent ref={contentRef}>
          ...
          <Wrapper id="swap-page">
            <AutoColumn gap="md">
              <CurrencyInputPanel
                label={
                  independentField === Field.OUTPUT && !showWrap && trade ? t('You Pay (estimated)') : t('You Pay')
                }
            ...
            <Box mt="1rem">
              {swapIsUnsupported ? (
                ...
                <Button width="100%" scale="lg" disabled={Boolean(wrapInputError)} onClick={onWrap}>
                  {wrapInputError ??
