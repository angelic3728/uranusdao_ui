...
interface LeaderboardRowProps {
  bidder: Bidder
  tanPriceUsdc: BigNumber
  isMobile: boolean
}

const LeaderboardRow: React.FC<LeaderboardRowProps> = ({ bidder, tanPriceUsdc, isMobile }) => {
        ...
        <Flex flexDirection="column" width="100%" justifyContent="flex-end" pr={[null, null, '24px']}>
          ...
          {tanPriceUsdc.gt(0) ? (
            <Text fontSize="12px" color="textSubtle" textAlign="right">
              ~${getBalanceNumber(amount.times(tanPriceUsdc)).toLocaleString('en', { maximumFractionDigits: 0 })}
            </Text>
          ) : (
    ...
    <Box>
      <LeaderboardContainer>
        ...
        {bidders.slice(0, visibleBidders).map((bidder) => (
          <LeaderboardRow key={bidder.account} bidder={bidder} tanPriceUsdc={tanPriceUsdc} isMobile={isMobile} />
        ))}
      </LeaderboardContainer>
