...
import { Modal, Text, Flex, BalanceInput, Box, Button, PancakeRoundIcon } from 'uranus-uikit'
...
import { useTan, useFarmAuctionContract } from 'hooks/useContract'
...
const PlaceBidModal: React.FC<PlaceBidModalProps> = ({
  ...
  useEffect(() => {
    if (userNotEnoughCake) {
      setErrorText(t('Insufficient TAN balance'))
    } else if (!isMoreThanInitialBidAmount && isFirstBid) {
      setErrorText(t('First bid must be %initialBidAmount% TAN or more.', { initialBidAmount }))
    ...
    <StyledModal title={t('Place a Bid')} onDismiss={onDismiss} headerBackground={theme.colors.gradients.cardHeader}>
      ...
      <InnerContent>
        ...
        <BalanceInput
          ...
          currencyValue={
            tanPriceUsdc.gt(0) &&
            `~${bid ? tanPriceUsdc.times(new BigNumber(bid)).toNumber().toLocaleString() : '0.00'} USD`
          }
        />
