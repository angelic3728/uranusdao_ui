...
import { Flex, Button, Text, AutoRenewIcon, PresentWonIcon } from 'uranus-uikit'
...
const ClaimInnerContainer: React.FC<ClaimInnerProps> = ({ onSuccess, roundsToClaim }) => {
  ...
  const tanReward = activeClaimData.cakeTotal
  const dollarReward = tanReward.times(tanPriceUsdc)
  const rewardAsBalance = getBalanceAmount(tanReward).toNumber()
