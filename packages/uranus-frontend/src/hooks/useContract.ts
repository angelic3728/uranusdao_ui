...
import { ChainId, WETH } from 'uranus-swap-sdk'
...
export const useTan = () => {
  const { library } = useActiveWeb3React()
  return useMemo(() => getTanContract(library.getSigner()), [library])
}
...
export const useTanVaultContract = () => {
  const { library } = useActiveWeb3React()
  return useMemo(() => getTanVaultContract(library.getSigner()), [library])
}
  ...
  if (chainId) {
    ...
    switch (chainId) {
      ...
      case ChainId.TESTNET:
        address = '0x00000000000C2E074eC69A0dFb2997BA6C7d2e1e' // FIXME
        break
    }
  }
