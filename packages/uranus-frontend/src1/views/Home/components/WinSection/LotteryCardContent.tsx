...
import { Flex, Text, Skeleton, Link, Button, ArrowForwardIcon } from 'uranus-uikit'
...
const LotteryCardContent = () => {
  ...
  const vvsPriceUsdcAsString = usePriceVvsUsdc().toString()

  const vvsPrizesText = t('%cakePrizeInUsd% in VVS prizes this round', { cakePrizeInUsd: vvsPriceUsdcAsString })
  const [pretext, prizesThisRound] = vvsPrizesText.split(vvsPriceUsdcAsString)

  const vvsPriceUsdc = useMemo(() => {
    return new BigNumber(vvsPriceUsdcAsString)
  }, [vvsPriceUsdcAsString])
  ...
  useEffect(() => {
    ...
    const fetchCurrentLotteryPrize = async () => {
      ...
      const prizeInUsdc = vvsPriceUsdc.times(amountCollectedInCake)
  ...
  }, [lotteryId, slowRefresh, setCurrentLotteryPrize, vvsPriceUsdc])
