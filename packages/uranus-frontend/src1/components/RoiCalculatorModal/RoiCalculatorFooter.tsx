...
const RoiCalculatorFooter: React.FC<RoiCalculatorFooterProps> = ({
  ...
  const {
    ...
    <>
      <Text>
        {t(
          'The Multiplier represents the proportion of VVS rewards each farm receives, as a proportion of the VVS produced each block.',
        )}
      </Text>
      <Text my="24px">
        {t('For example, if a 1x farm received 1 VVS per block, a 40x farm would receive 40 VVS per block.')}
      </Text>
      ...
      {isExpanded && (
        <Box px="8px">
          <Grid gridTemplateColumns="2.5fr 1fr" gridRowGap="8px" gridTemplateRows={`repeat(${gridRowCount}, auto)`}>
            ...
            <Text color="textSubtle" small>
              {isFarm ? t('Base APR (VVS yield only)') : t('APR')}
            </Text>
            <Text small textAlign="right">
              {apr.toFixed(2)}%
            </Text>
